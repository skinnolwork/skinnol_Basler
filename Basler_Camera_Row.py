...
class BaslerCamera:
    ...
    # 낙폭 기준 추가 함수
    # def filter_peaks_by_falloff(self, peaks, intensity, falloff_threshold=0.5, check_range=80):
    #     valid_peaks = []
    #     for peak in peaks:
    ...
    def save_image(self, row_intensity, img_array):
        ...
        filename_image = f"./capture_{self.mono_mode}_Row_{timestamp}.{'bmp' if self.mono_mode == 'Mono8' else 'tiff'}"
        # 카메라 저장
        ...
        # 그래프 저장
        filename_graph = f"./graph_{self.mono_mode}_Row_{timestamp}.png"
        ...
        filename_csv = f"./data_{self.mono_mode}_Row_{timestamp}.csv"

...
class BaslerCamera:
    ...
    def setup_figure(self):
        self.fig, (self.ax1, self.ax2, self.ax3) = plt.subplots(3, 1, figsize=(6, 5))

        # 미리 라인 설정 (빈 데이터로 초기화)
        self.line1, = self.ax1.plot([], [], color='red', linewidth=1)
        self.line2, = self.ax2.plot([], [], color='red', linewidth=1)
        self.line3, = self.ax3.plot([], [], color='red', linewidth=1)

        # inset 그래프 추가
        self.ax_inset = inset_axes(
            self.ax1,
            width="80%", height="100%",  # 비율로 inset 크기 조절
            bbox_to_anchor=(0.65, 0.5, 0.5, 0.5),  # 위치(x, y), 크기(w, h) 모두 비율로
            bbox_transform=self.ax1.transAxes,
            loc='upper right',
            borderpad=0
        )

        self.inset_line, = self.ax_inset.plot([], [], color='black', linewidth=0.5)
        ...
        try:
            ...
            if hasattr(manager, "window"):
                manager.window.move(150, 150)  # x=100, y=100 → 왼쪽 상단
    ...
    def display_camera_feed(self, img_array, row_intensity):
        ...
        if self.use_cm: 
            ...
            ax.plot(self.x_cm[:third], row_intensity[:third], color='red', linewidth=1)
            ...
            # inset axes 추가 (항상 전체 데이터 표현)
            ax_inset = inset_axes(ax, width="40%", height="90%", loc='upper right')
            ax_inset.plot(x_axis, row_intensity, 'k-', linewidth=1)
            ax_inset.grid()

            ax_inset.set_xlim(x_axis[0], x_axis[-1])
            ax_inset.set_ylim(0, y_max)
            ax_inset.tick_params(axis='both', colors='blue') 


            # ▶️ 두 번째 subplot
            ax = plt.subplot(3, 1, 2)
            ax.plot(self.x_cm[third:2*third], row_intensity[third:2*third], color='red', linewidth=1)
            ax.set_ylabel("Intensity")
            ax.set_ylim([0, y_max])
            ax.xaxis.set_major_locator(MultipleLocator(100))
            ax.xaxis.set_minor_locator(MultipleLocator(20))
            ...
            ax.plot(self.x_cm[2*third:], row_intensity[2*third:], color='red', linewidth=1)
            ax.set_ylabel("Intensity")
            ax.set_ylim([0, y_max])
